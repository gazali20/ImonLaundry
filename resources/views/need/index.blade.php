<x-layout.default>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                needs: @json($needs->toArray()),
                dataTable: null,
                init() {

                    const needEditUrl = "{{ route('need.edit', ['need' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["ID", "Kategori", "Aksi"],
                            data: this.needs.map((need, index) => {
                                const editneed = needEditUrl.replace(':id', need.id);
                                return [
                                    index + 1,
                                    need.name_category,
                                    `<div class="flex items-center gap-2">
                                            ...
                                            <button @click.prevent="confirmDelete(${need.id})" type="button">
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                async confirmDelete(needId) {
                    const isConfirmed = await this.showConfirmDialog();
                    console.log(isConfirmed);

                    if (isConfirmed) {
                        this.deleteNeed(needId);
                    }
                },
                async deleteNeed(needId) {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

                    const route = "{{ route('need.destroy', ':id') }}";

                    

                    const url = route.replace(':id', needId);
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            this.removeNeedFromList(needId);
                ...
                removeNeedFromList(needId) {

                    const index = this.needs.findIndex(data => data.id === needId);
                    if (index !== -1) {
