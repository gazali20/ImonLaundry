    ...
    <div class="pt-3" x-data="form">
        <div class="panel max-w-xl">
            <h2 class="text-lg font-semibold mb-4 flex items-center space-x-2">
                <a href="/category" class="inline-flex items-center">
            ...
            <form @submit.prevent="submitForm()">
                <div class="grid grid-cols-1  gap-5">
                    ...
                    <div :class="[isSubmitForm ? (form.name_category ? 'has-success' : 'has-error') : '']">
                        ...
                        <input id="custoName" type="text" placeholder="Masukan Kategori Layanan"
                            class="form-input w-full" x-model="form.name_category" />
                        <template x-if="isSubmitForm && form.name_category">
                            <p class="text-success mt-1">Kategori layanan terisi</p>
                        </template>
                        <template x-if="isSubmitForm && !form.name_category">
                            <p class="text-danger mt-1">Harap isi kategori layanan!</p>
                        </template>
                    </div>
                </div>

                <button type="submit" class="btn bg-purple-600 hover:bg-purple-700 text-white mt-6">Simpan</button>
            </form>
        </div>
    </div>
    <script>
