<x-layout.default>
    ...
    <ul class="flex space-x-2 rtl:space-x-reverse">
        ...
        <li>
            <a href="/need"
                class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1 text-purple-500 hover:underline">Kebutuhan</a>
        </li>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                categories: @json($category->toArray()),
                dataTable: null,
                init() {

                    const categoryEditUrl = "{{ route('category.edit', ['category' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["ID", "Kategori", "Aksi"],
                            data: this.categories.map((category, index) => {
                                const editCategory = categoryEditUrl.replace(':id', category
                                    .id);
                                return [
                                    index + 1,
                                    ...
                                    `<div class="flex items-center gap-2">
                                            ...
                                            <button @click.prevent="confirmDelete(${category.id})" type="button">
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
