<div class="bg-white p-4 rounded-lg shadow mt-6">
    <h3 class="text-lg font-semibold mb-2">Sedang Dicuci</h3>
    <table class="w-full border-collapse">
        ...
        <tbody>
            @foreach ($dicuci as $kasir)
                @foreach ($kasir->kasirService as $item)
                    <tr>
                        <td class="p-3">{{ $kasir->customer }}</td>
                        <td class="p-3">{{ $kasir->no_handphone }}</td>
                        <td class="p-3">{{ $kasir->grand_total }}</td>
                        <td class="p-3">{{ $item->weight }} kg</td>
                        <td class="p-3">{{ $kasir->payment }}</td>
                        <td class="p-3">{{ $kasir->date }}</td>
                        <td class="p-3"><a href="{{ route('kasir.rincian', $kasir->id) }}"><div class="p-2 hover:bg-purple-200 rounded-md inline-block" ><svg   width="20" height="23" viewBox="0 0 27 30" fill="none" xmlns="http://www.w3.org/2000/svg">
