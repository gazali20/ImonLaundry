<x-layout.default>
    <h5 class="font-semibold text-lg dark:text-white-light">List Invoice</h5>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="border-t {{ $loop->iteration % 2 == 0 ? 'bg-gray-100' : '' }}">
                    <td class="font-semibold text-purple-600"><a href="#">{{ $item->code_invoice }}</a></td>
                        <td>{{ $item->date }}</td>
                        <td>{{ $item->customer }}</td>
                        <td>{{ $item->no_handphone }}</td>
                        <td>Rp {{ number_format($item->grand_total, 2, ',', '.') }}</td>
                        <td>
                            <a href="{{ route('Invoice.detail', $item->id) }}" class="hover:bg-purple-200 p-2 rounded-md inline-block">
