<x-layout.default>
    <div x-data="striped" class="panel">
        <div class="flex justify-between mb-6">
            <h5 class="font-semibold text-lg dark:text-white-light">Spare Parts</h5>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                ...
                dataTable: null,
                init() {
                    const sparePartEditUrl = "{{ route('spare_part.edit', ['spare_part' => ':id']) }}";
                    const sparePartDestroyUrl =
                        "{{ route('spare_part.destroy', ['spare_part' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["ID", "Nama", "Harga", "Jumlah", "Aksi"],
                            data: this.spareParts.map((sparePart, index) => {
                                const editSparePart = sparePartEditUrl.replace(':id',
                                    sparePart.id);
                                const deleteSparePart = sparePartDestroyUrl.replace(':id',
                                    sparePart.id);
                                return [
                                    index + 1,
                                    sparePart.name,
                ...
                async confirmDelete(sparePartId) {
                    ...
                    if (isConfirmed) {
                        this.deleteSparePart(sparePartId);
                    }
                },
                ...
                async deleteSparePart(sparePartId) {
                    ...
                    const route = "{{ route('spare_part.destroy', ':id') }}";
                    const url = route.replace(':id', sparePartId);
